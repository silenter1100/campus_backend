...
impl UserService {
    ...
    pub async fn update_profile(
        ...
        if let Some(setting_theme) = update_data.setting_theme {
            user.setting_theme = Some(setting_theme);
        }
        if let Some(setting_privacy_course) = update_data.setting_privacy_course {
            user.setting_privacy_course = Some(setting_privacy_course);
        }
        if let Some(setting_notification_switch) = update_data.setting_notification_switch {
            user.setting_notification_switch = Some(setting_notification_switch);
        }
    ...
    pub async fn change_password(
        ...
        // 更新密码（未哈希）
        let new_password_hash = new_password.to_string();
    ...
    /// 退出登录
    pub async fn logout(_pool: &MySqlPool, _user_id: &str) -> Result<(), AppError> {
        Ok(())
    }
}
