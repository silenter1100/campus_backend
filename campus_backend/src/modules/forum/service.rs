...
impl ForumService {
    ...
    // Board
    // =========================================================================
    pub async fn get_board_list(state: &AppState) -> Result<Vec<BoardVO>, AppError> {
        let pool = &state.pool;
    ...
    pub async fn update_post(
        ...
        if let Some(media) = req.media {
            sqlx::query!("DELETE FROM post_medias WHERE post_id = ?", post_id)
                .execute(&mut *tx)
                .await?;
    ...
    pub async fn get_post_list(
        ...
        qb.push(") as is_liked, ");
        qb.push(
            "EXISTS(SELECT 1 FROM post_collections pc WHERE pc.post_id = p.id AND pc.user_id = ",
        );
    ...
    pub async fn get_post_detail(
        ...
        let rows = sqlx::query!(
            ...
            FROM post_medias WHERE post_id = ?
            "#,
            post_id
        )
            .fetch_all(pool)
    ...
    pub async fn delete_post(
        ...
        let result = sqlx::query!(
            "UPDATE posts SET is_deleted = 1 WHERE id = ? AND author_id = ?",
            post_id,
            user_id
        )
            .execute(pool)
        ...
        if result.rows_affected() == 0 {
            return Err(AppError::Forbidden(
                "Cannot delete post or post not found".into(),
            ));
        }
    ...
    pub async fn toggle_like_post(
        ...
        let is_liked = if action == "like" {
            ...
            sqlx::query!(
                "UPDATE post_stats SET like_count = GREATEST(like_count - 1, 0) WHERE post_id = ?",
                post_id
            )
                .execute(&mut *tx)
                .await?;
    ...
    // Post - Toggle Collect
    // =========================================================================
    pub async fn toggle_collect_post(
        state: &AppState,
        post_id: &str,
        user_id: &str,
        action: &str
        ...
        if action == "collect" {
            sqlx::query!(
                "INSERT IGNORE INTO post_collections (post_id, user_id) VALUES (?, ?)",
                post_id,
                user_id
            )
                .execute(pool)
    ...
    pub async fn admin_list_reports(
    ...
    ) -> Result<Pagination<serde_json::Value>, AppError> {
        let pool = &state.pool;
        ...
        for row in rows {
            ...
            let (snippet, author) = if target_type == "post" {
                (
                    row.get::<Option<String>, _>("post_content").unwrap_or_default(),
                    row.get::<Option<String>, _>("post_author_name").unwrap_or_default(),
                )
            } else {
                (
                    row.get::<Option<String>, _>("comment_content").unwrap_or_default(),
                    row.get::<Option<String>, _>("comment_author_name").unwrap_or_default(),
                )
            };
